...
func (h *ImportTaskHandler) processBatch(ctx context.Context, taskID uuid.UUID, objTypeID string, batch []ImportDataRow, 
    ...
    for _, row := range batch {
        ...
        obj, err := h.queries.GetObjectByIDString(ctx, row.IDString)
        if err != nil && err != sql.ErrNoRows {
        ...
        valuesJSON, _ := json.Marshal(row.Values)
        _, err = h.queries.UpsertObjectTypeValue(ctx, database.UpsertObjectTypeValueParams{
            ObjID:    obj.ID,
            TypeID:   uuid.MustParse(objTypeID),
            TypeValues:   valuesJSON,
        })
        if err != nil {
            return fmt.Errorf("failed to upsert object type value: %w", err)
        }
				newFact, err := h.queries.CreateFact(ctx, database.CreateFactParams{
					Text:       fact.Text,
				...
				h.queries.AddObjectsToFact(ctx, database.AddObjectsToFactParams{
					Column1: objectIds,
					FactID: newFact.ID,
					OrgID: OrgId,
				})
