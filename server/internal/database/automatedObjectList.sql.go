...
// versions:
//   sqlc v1.27.0
// source: automatedObjectList.sql
...
const listMatchingObjectIDs = `-- name: ListMatchingObjectIDs :many
WITH object_data AS (
...
type ListMatchingObjectIDsParams struct {
...
type ListMatchingObjectIDsRow struct {
	ID        uuid.UUID   `json:"id"`
	TagIds    interface{} `json:"tag_ids"`
	FunnelIds interface{} `json:"funnel_ids"`
}

func (q *Queries) ListMatchingObjectIDs(ctx context.Context, arg ListMatchingObjectIDsParams) ([]ListMatchingObjectIDsRow, error) {
	rows, err := q.query(ctx, q.listMatchingObjectIDsStmt, listMatchingObjectIDs,
		arg.OrgID,
		arg.Column2,
	...
	defer rows.Close()
	var items []ListMatchingObjectIDsRow
	for rows.Next() {
		var i ListMatchingObjectIDsRow
		if err := rows.Scan(&i.ID, &i.TagIds, &i.FunnelIds); err != nil {
			return nil, err
		}
