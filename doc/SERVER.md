...
This server is an api server which return json for each route. This server is written in Go lang. This server serve as stateless CRUD for the database.

There are 4 major components of this project: Authentication, Setting, Data and Workplace.
...
Authentication is the module where all organisation and user registration happen. This module also handle login provide session jwt and protect most route with correct permission.

Every member of an organisation can change data within that organisation. For example, if a member in a orgnisation query tag or object, it will show everything belong to the organisation regardless of creator.
However, only admin role can add new member.

Except login and register, every route should be protected by authentication using the creator_session table. This module will read `creator_session` in the request header. If session is invalid then it will return a http code for invalid access.
...
This module holds data structure and meta data. This consists of tag, object type, funnel and its steps, list or view configuration.
...
This module hold access to all tasks, objects and creator lists which is saved filter setting of generic lists.
...
This module handle the view (creator_list) of a creator, his task and facts relating to an object.
